...
let configKeepKey = {
  pairingInfo:{
    ...
    imageUrl: process.env['SERVICE_IMAGE_URL'] || 'https://assets.coincap.io/assets/icons/dgb@2x.png',
    basePath:spec,
...
function App() {
  ...
  let onSend = async function(){
    try{
      ...
      let inputs = await pioneer.ListUnspent({network:'DGB',xpub})
      inputs = inputs.data
  ...
  return (
    <ChakraProvider theme={theme}>
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Sending DGB</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <div>
        ...
        <Grid minH="100vh" p={3}>
          ...
          <VStack spacing={8}>
            ...
            {isLoading ? <div>loading...</div> : <div>
              ...
              <Button onClick={onOpen}>Send DGB</Button>
            </div>}
          </VStack>
        </Grid>
      </Box>
